...
const { loadHome, loadFrequent } = require('./home');
const { displayMenu } = require('./menu');
const { updateLeftPanel } = require('./leftPanel');
const { loadCategories } = require('./categoriesList');
const { loadOrderHistory } = require('./history');
const { loadCategoryHistory, fetchCategoryWise } = require('./categoryWiseTable');
const { loadTodaysOrders } = require('./todaysOrders');
const { fetchDeletedOrders } = require('./deletedOrdersTable');
const { fetchDiscountedOrders, clearDiscountedOrders } = require('./discountedOrders');
const { loadCustomers } = require('./customer');
const { loadSearchOrder } = require('./searchOrder');
...
const { loadDayEndSummary } = require('./dayEndSummary');
...
const { loadHelpSection } = require('./help');
...
//--------------------------------------- CATEGORY PANEL AND TOP PANEL SESSION STORAGE STARTS HERE ----------------------------------------
    ...
    const settingsButtons = [
        ...
        'ConnectedDevices', 'Help', 'customizeLeftPanel', 'Switches', 'ChangeMasterPassword', 'ArrangeMenu'
    ];
...
//--------------------------------------- CATEGORY PANEL AND TOP PANEL SESSION STORAGE ENDS HERE ----------------------------------------
    ...
    <div class="food-items" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
        ...
        else if (contentType === "ArrangeMenu") {
            loadArrangeMenu(mainContent, billPanel);
        }
